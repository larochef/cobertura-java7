...
public class SecondPassMethodInstrumenter extends NewLocalVariableMethodAdapter {
    ...
    public void visitLabel(Label label) {
        ...
        if (firstPass.getJumpTargetLabels().keySet().contains(label)) { //this label is the true branch label
            if (lastJump != null) { //this is also label after jump - we have to check the branch number whether this is the true or false branch
        ...
        } else if (lastJump != null) { //this is "only" after jump label, hit the false branch only if the lastJump is same as stored stack lineNumber and jumpNumber
            Label newLabelX = instrumentIsLastJump();
